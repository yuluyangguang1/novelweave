...
只读 `world[].lifecycle['destroyed-in']`。这个字段由 `nw-changes.mjs` 的 `world.destroy` 算子写入、
导出与三方合并一路带着走，**此前没有任何规则读它**——记了不查等于没记，这条就是把那半截接通。
取条目本名与 `keys` 里 ≥2 字的称呼，在销毁章**之后**的正文字面检索，命中即报；
...
角色卡与世界条目一起查（称呼取 `nameForms` 与 `name`/`keys`/`secondary_keys`）。
