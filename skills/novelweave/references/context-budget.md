...
4. `未结线索` —— 未回收伏笔与开放悬念
5. `前情摘要` —— 之前最多 12 章，每章一行（取 `summary` 的「核心事件」，≤80 字）
6. `相关世界设定` —— 关键词触发命中的条目
7. `本章已有正文` —— 最后 1500 字（有正文时排在前头）
8. `上章尾部` —— 最后 1200 字

第 2 位放角色卡并把死亡警示写进去，是有意的：模型不知道"这个人不能行动"，
就会让他自然地走出来。这是 R1 类矛盾最常见的产生方式。

第 7、8 两位会**互换**：本章已经有正文时，接着写比回读上一章结尾更要紧，
所以把「本章已有正文」提到前面 —— 预算一紧被裁掉的应该是后者。

`前情摘要` 每章只留一行，不是省事：位置/伤势另有「分章状态快照」、伏笔另有「未结线索」，
12 章各 200 字全塞进来约 7KB，会把整份 12288 字节预算吃空，最后挤掉的恰恰是要接着写的那段。
